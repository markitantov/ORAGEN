...
class AGenderClassificationHead(nn.Module):
    ...
    def __init__(self, input_size=256, output_size=4):
        super(AGenderClassificationHead, self).__init__()
...
class AGenderClassificationHeadV2(nn.Module):
    """AGenderClassificationHead"""
    def __init__(self, input_size=256, output_size=4):
        super(AGenderClassificationHeadV2, self).__init__()
        self.fc_gender = nn.Linear(input_size, 2)
        self.fc_age = nn.Linear(input_size, 1)

    def forward(self, x_gender, x_age):
        x_gender = self.fc_gender(x_gender)
        x_age = self.fc_age(x_age)
        return {'gen': x_gender, 'age': x_age.squeeze()}
    
    
class MaskAGenderClassificationHead(nn.Module):
    """AGenderClassificationHead"""
    def __init__(self, input_size=256, output_size=3):
        super(MaskAGenderClassificationHead, self).__init__()
        self.fc_agender = nn.Linear(input_size, output_size)
        self.fc_mask = nn.Linear(input_size, 6)
    ...
    def forward(self, x):
        ...
        x_masktype = self.fc_mask(x)
        return {'gen': x_gen, 'age': x_age, 'mask': x_masktype}


class Permute(nn.Module):
    """Some Information about Permute"""
